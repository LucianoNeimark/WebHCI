...
export class RoomsApi{
    ...
    static async removeDeviceFromRoom(roomId : string, deviceId : string) {
        const res = await Api.delete(`/rooms/${roomId}/devices/${deviceId}`)
        const { result } = await res.json()
        return result
    }

    static async moveDeviceToRoom(oldRoomId : string, newRoomId: string, deviceId : string) {
        await this.removeDeviceFromRoom(oldRoomId, deviceId)
        const res = await this.addDeviceToRoom(newRoomId, deviceId)
        const { result } = await res.json()
        return result
    }
}
